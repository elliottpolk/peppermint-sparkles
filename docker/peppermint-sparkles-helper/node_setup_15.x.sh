...
#   http://github.com/nodesource/distributions
#
...
#   or
# wget -qO- http://deb.nodesource.com/setup_15.x | bash -
#
# CONTRIBUTIONS TO THIS SCRIPT
#
# This script is built from a template in
# http://github.com/nodesource/distributions/tree/master/deb/src
...
${bold}${NODENAME} is no longer actively supported!${normal}
   ...
   * ${green}http://deb.nodesource.com/setup_10.x â€” Node.js 10 LTS \"Dubnium\"${normal}
   * ${green}http://deb.nodesource.com/setup_12.x â€” Node.js 12 LTS \"Erbium\"${normal} (recommended)
   * ${green}http://deb.nodesource.com/setup_14.x â€” Node.js 14 LTS \"Fermium\"${normal}
   * ${green}http://deb.nodesource.com/setup_15.x â€” Node.js 15 \"Fifteen\"${normal}

  Please see ${bold}http://github.com/nodejs/Release${normal} for details about which
  version may be appropriate for you.

  The ${bold}NodeSource${normal} Node.js distributions repository contains
  information both about supported versions of Node.js and supported Linux
  distributions. To learn more about usage, see the repository:
    ${bold}http://github.com/nodesource/distributions${normal}
"
        echo
...
# (Check snaked from: http://get.docker.io/ubuntu/)

if [ ! -e /usr/lib/apt/methods/http ]; then
    PRE_INSTALL_PKGS="${PRE_INSTALL_PKGS} apt-transport-http"
fi
...
if [[ $IS_PRERELEASE -eq 0 ]]; then
    print_status "Your distribution, identified as \"$(lsb_release -d -s)\", is a pre-release version of Ubuntu. NodeSource does not maintain official support for Ubuntu versions until they are formally released. You can try using the manual installation instructions available at http://github.com/nodesource/distributions and use the latest supported Ubuntu version name as the distribution identifier, although this is not guaranteed to work."
    exit 1
fi
...
if [ -x /usr/bin/curl ]; then
    exec_cmd_nobail "curl -k  -sLf -o /dev/null 'http://deb.nodesource.com/${NODEREPO}/dists/${DISTRO}/Release'"
    RC=$?
else
    exec_cmd_nobail "wget --no-check-certificate -qO /dev/null -o /dev/null 'http://deb.nodesource.com/${NODEREPO}/dists/${DISTRO}/Release'"
    RC=$?
fi

if [[ $RC != 0 ]]; then
    print_status "Your distribution, identified as \"${DISTRO}\", is not currently supported, please contact NodeSource at http://github.com/nodesource/distributions/issues if you think this is incorrect or would like your distribution to be considered for support"
    exit 1
fi
...
if [ -x /usr/bin/curl ]; then
    exec_cmd 'curl -k -s http://deb.nodesource.com/gpgkey/nodesource.gpg.key | apt-key add -'
else
    exec_cmd 'wget --no-check-certificate -qO- http://deb.nodesource.com/gpgkey/nodesource.gpg.key | apt-key add -'
fi
...
## To install the Yarn package manager, run:
     curl -k -sL http://dl.yarnpkg.com/debian/pubkey.gpg | sudo apt-key add -
